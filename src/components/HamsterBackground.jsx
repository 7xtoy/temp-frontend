...
const FallingHamster = ({explosionRef, setCounter}) => {
    ...
    return (
        <motion.div
            ...
            key={key}
            onClick={() => handleClick()}
            className={clsx("w-24 h-fit absolute cursor-crosshair select-none z-[-1]", {"hidden": isClicked})}
            initial={{
                top: "-15%",
                left: `${leftOffset}%`,
                rotateX: 0                   
        ...
        >
            <img src={Hamster} alt="Just a hamster..." className="h-fit w-full pointer-events-none"/>
        </motion.div>
    )
}
    ...
    return (
        <div className={clsx(
            ...
            <img 
                ref={explosionRef}
                src={Explosion} 
                className={"w-24 absolute top-auto -translate-y-1/2 select-none pointer-events-none"}
                style={{display: "none"}}
            />

            <p className="text-3xl absolute bottom-0"><ToyTitle title={`Hamster count: ${counter}`} /></p>
        </div>
    )
}
